...
fit_dcmp <- prod_train %>%
  model(stlf = decomposition_model(
    STL(prod_pet ~  trend(window =3)+ season(window=7)
        ,robust = TRUE),
    NAIVE(season_adjust)
  ))
...
prophet_df_train = prophet_df %>% filter(year(ds) <= 2019)
...
future$precio_interno_crudo = prophet_df[year(prophet_df$ds)==2020,]$"precio_interno_crudo"
future$precio_interno_gas = prophet_df[year(prophet_df$ds)==2020,]$"precio_interno_gas"
future$inversion_explotacion =
  prophet_df[year(prophet_df$ds)==2020,]$"inversion_explotacion"
